...
export function DueDiligenceSection() {
  ...
  return (
    <section className="relative w-full bg-white font-sans selection:bg-blue-200 flex flex-col items-center overflow-x-hidden pt-4 pb-16 px-4 sm:px-6">
      <div className="mx-auto flex w-full max-w-[22.5rem] sm:max-w-[25.5rem] flex-col items-center">
        ...
        {/* Section Heading */}
        <h2 className="mt-2.5 text-center text-[1.3rem] min-[390px]:text-[1.45rem] sm:text-[1.65rem] font-black leading-tight tracking-tight text-[#0c1e38]">
          Modern Due Diligence &amp; <br />
          <span className="text-[#1665f5]">The Unfiltered Narrative</span>
        </h2>

        {/* Subheadline with proper line separation */}
        <p className="mt-2 text-center text-[0.78rem] min-[390px]:text-[0.84rem] sm:text-[0.88rem] font-normal leading-[1.4] text-[#64748b]">
          Search our name, check Reddit, or audit our locations.<br />
          We believe in total radical transparency before you attend.
        </p>

        {/* Streamlined Social Links (No Nested Card or Header) */}
        <div className="mt-5 grid w-full grid-cols-3 gap-2">
          <a
            href="https://youtube.com"
            target="_blank"
            rel="noreferrer"
            className="group flex h-10 items-center justify-center gap-1.5 rounded-xl border border-[#e2e8f0] bg-white px-2 shadow-2xs transition-all hover:border-[#1665f5] hover:bg-[#edf4ff]/50 active:scale-[0.97]"
          >
            ...
            <span className="text-[0.74rem] sm:text-[0.78rem] font-bold tracking-tight text-[#0c1e38] transition-colors group-hover:text-[#1665f5]">
              YouTube
            </span>
          </a>
        ...
        {/* Plain Text Accordion List (No Cards, Minimal Dividers) */}
        <div className="mt-6 w-full divide-y divide-[#e2e8f0] border-y border-[#e2e8f0]">
          {DUE_DILIGENCE_DATA.map((item) => {
            ...
            return (
              <div key={item.id} className="py-3.5 sm:py-4">
                ...
                >
                  <span
                    className={`text-[0.92rem] sm:text-[1rem] font-bold tracking-tight transition-colors ${
                      isOpen ? "text-[#1665f5]" : "text-[#0c1e38] group-hover:text-[#1665f5]"
                    }`}
                  >
                    {item.title}
                  </span>
                  ...
                  <span
                    className={`ml-3 shrink-0 text-[#64748b] transition-transform duration-200 ${
                      isOpen ? "rotate-180 text-[#1665f5]" : ""
                    }`}
                  >
                    <svg
                      className="h-4 w-4"
                ...
                {/* Accordion Content */}
                {isOpen && (
                  <div className="pt-3 pb-1 space-y-3.5 text-[0.8rem] sm:text-[0.85rem] leading-relaxed text-[#475569]">
                    {item.questions.map((faq, idx) => (
                      <div key={idx} className="space-y-1">
                        <p className="font-bold text-[#0c1e38]">{faq.q}</p>
                        <p>{faq.a}</p>
                      </div>
                    ))}

                    {/* Google Maps Hub Links for Legitimacy */}
                    {item.id === "legitimacy" && (
                      <div className="pt-2">
                        <p className="text-[0.72rem] font-bold uppercase tracking-wider text-[#64748b]">
                          Verified Locations:
                        </p>
                        <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1.5 text-[0.76rem]">
                          {MAP_LOCATIONS.map((loc) => (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="font-medium text-[#1665f5] hover:underline"
                            >
        ...
        <div className="mt-6 w-full rounded-[1.35rem] border border-[#eedac2] bg-[#fcf8f2] p-4 shadow-2xs">
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col">
              <h4 className="text-[0.84rem] sm:text-[0.88rem] font-black text-[#0c1e38] leading-tight">
                Our Radical Transparency Guarantee
              </h4>
              <p className="mt-1 text-[0.74rem] sm:text-[0.78rem] text-[#64748b] leading-[1.38]">
