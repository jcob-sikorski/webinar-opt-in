...
const BONUSES: BonusItem[] = [
  {
    ...
    icon: (
      <svg className="h-6 w-6 text-[#b5854b]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
...
export function SpecialOfferBonuses() {
  return (
    <section className="relative w-full bg-white py-12 px-4 sm:px-6">
      <div className="mx-auto flex w-full max-w-[23rem] sm:max-w-[27rem] flex-col items-center">
        
        {/* Section Pill */}
        <div className="inline-flex items-center justify-center rounded-full border border-[#eedac2] bg-[#fcf8f2] px-3.5 py-0.5 shadow-2xs">
          <span className="text-[0.66rem] font-bold uppercase tracking-[0.16em] text-[#b5854b]">
            EXCLUSIVE VAULT
          </span>
        </div>

        {/* Section Heading */}
        <h2 className="mt-3 text-center text-[1.4rem] min-[390px]:text-[1.6rem] sm:text-[1.85rem] font-black leading-tight tracking-tight text-[#0c1e38]">
          Plus, When You Sign Up Get <br />
          <span className="text-[#1665f5]">These Exclusive Bonuses!</span>
        </h2>
        ...
        <div className="mt-6 flex w-full flex-col gap-4">
          {BONUSES.map((bonus) => (
            <div
              key={bonus.number}
              className="w-full rounded-[1.4rem] border border-[#eedac2] bg-white p-4 sm:p-5 shadow-2xs"
            >
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[#eddac2] bg-[#fcf8f2]">
                  {bonus.icon}
                </div>
                <div className="flex flex-col">
                  <span className="text-[0.68rem] font-extrabold uppercase tracking-wider text-[#b5854b]">
                    {bonus.number}
                  </span>
                  <h3 className="text-[1.02rem] sm:text-[1.12rem] font-black text-[#0c1e38] leading-tight">
                    {bonus.title}
                  </h3>
                </div>
              </div>

              <div className="mt-3.5 space-y-2 border-t border-[#f7ede0] pt-3 text-[0.79rem] sm:text-[0.84rem] text-[#475569] leading-[1.44]">
